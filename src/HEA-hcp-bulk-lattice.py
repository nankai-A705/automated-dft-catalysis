...
def get_random_hcp_alloy(num_structure, host, repeat, eps, **kwargs):
    ...
    for i in range(num_structure):
        ...
        for a in repeat* a0 * np.linspace(1 - eps, 1 + eps,5):
            for c in repeat* c0 * np.linspace(1 - eps, 1 + eps,5):
                super_cell.set_cell([a,a,c,angles[0],angles[1],angles[2]], apply_constraint=False)
