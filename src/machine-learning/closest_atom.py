...
import numpy as np
from tools import get_polygon_area, distance_between_coordinate
...
def get_surface_zone_area_near_ads(atoms, tolrence=0.8, adsorbate=None):
    ...
    # To make sure the area with same order we fix are in hexagon zone
    ...
    if dist[1] - dist[0] > tolrence and dist[7] - dist[6] > tolrence: #  top
       ...
       area_atom_position_xy = [atoms[i].position[:2] for i in idx ]
        ...
        idx = idx[:3]
        area_atom_position_xy = [atoms[i].position[:2] for i in idx ] * 6
    elif dist[4] - dist[2] > tolrence and dist[1] - dist[0] < 0.3 and dist[3]  - dist[2] <0.3: # bridge
        idx = idx[:4]
        area_atom_position_xy = [atoms[i].position[:2] for i in idx ] * 3 
    else:
        idx = idx = idx[:4]
        area_atom_position_xy = [atoms[i].position[:2] for i in idx ] * 3 

    ads_position_xy = adsorbates_center(atoms)[:2]
    
    return get_polygon_area(area_atom_position_xy), idx
...
def encode_atoms_number_near_ads(atoms, cutoff):
    """
...
#i  = (Cu, 20,1, 4, [6,3,3])    
print(encode_atoms_number_near_ads(Cu, 5))
#print(Cu.tag)
